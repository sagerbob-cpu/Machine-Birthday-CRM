...
import { Mail, Calendar, Settings, CheckCircle, Clock, Users, Plus, X, Download, MapPin, Edit2, Trash2, AlertTriangle, Loader2, Info, Building2, Menu } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      } catch (err) { setStatusMsg("Connection Failed."); setLoading(false); }
    };
    initAuth();
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-900 selection:bg-indigo-100 relative overflow-hidden">
      ...
      {/* Sidebar (Desktop) */}
      <div className="hidden md:flex w-64 bg-indigo-950 text-white p-6 flex flex-col shadow-2xl z-20">
        <SidebarContent />
      </div>
      ...
      {/* Main Content */}
      <div className="flex-1 p-4 md:p-10 overflow-auto z-10 relative">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6">
          ...
          <div className="flex flex-row w-full lg:w-auto gap-3">
            <button onClick={handleExportCSV} className="flex-1 lg:flex-none bg-white border-2 border-slate-200 px-6 py-4 md:py-3 rounded-2xl font-black shadow-sm hover:bg-slate-50 active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest text-xs md:text-sm text-slate-600">
              <Download size={18} /> Export Labels
            </button>
            <button onClick={() => { setEditingId(null); setFormData({customer:'', contact:'', machine:'', purchaseDate:'', lifespanYears:5, address:'', city:'', state:'', zip:''}); setShowModal(true); }} className="flex-1 lg:flex-none bg-indigo-600 text-white px-6 py-4 md:py-3 rounded-2xl font-black shadow-xl hover:bg-indigo-700 active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest text-xs md:text-sm">
        ...
        <div className="hidden md:block bg-white/80 backdrop-blur-sm rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden mb-12">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50 text-[11px] font-bold uppercase tracking-widest text-slate-400 border-b">
              <tr><th className="p-6">Customer / Address</th><th className="p-6">Machine Details</th><th className="p-6 text-center">Life Stage</th><th className="p-6 text-right">Action</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {dashboardData.toSend.length === 0 ? (
                <tr><td colSpan="4" className="p-20 text-center text-slate-300 font-bold italic text-lg uppercase opacity-50">Queue Empty</td></tr>
              ) : (
                dashboardData.toSend.map(item => (
                  <tr key={item.id} className="hover:bg-indigo-50/30 transition-colors group">
                    <td className="p-6">
                      <div className="flex items-center gap-3 mb-1"><p className="font-bold text-slate-900 text-lg leading-none truncate max-w-[250px]"><SafeVal value={item.customer} /></p><div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => { setEditingId(item.id); setFormData(item); setShowModal(true); }} className="text-slate-300 hover:text-indigo-600 p-2 bg-white border border-slate-100 rounded-xl shadow-sm transition-all"><Edit2 size={14}/></button><button onClick={() => setDeleteConfirmId(item.id)} className="text-slate-300 hover:text-red-500 p-2 bg-white border border-slate-100 rounded-xl shadow-sm transition-all"><Trash2 size={14}/></button></div></div>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1"><MapPin size={10} className="text-indigo-400" /> <SafeVal value={item.address} />, <SafeVal value={item.city} /> <SafeVal value={item.state} /></p>
                    </td>
        ...
        <div className="md:hidden space-y-4 mb-12">
          {dashboardData.toSend.length === 0 ? (
            <div className="p-12 text-center text-slate-400 font-black uppercase italic">Nothing due today</div>
          ) : (
            dashboardData.toSend.map(item => (
              <div key={item.id} className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="font-black text-xl text-slate-900 leading-none mb-1"><SafeVal value={item.customer} /></h3>
                    <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest flex items-center gap-1"><MapPin size={10}/> {item.city}, {item.state}</p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => { setEditingId(item.id); setFormData(item); setShowModal(true); }} className="p-2 bg-slate-50 rounded-xl text-slate-400"><Edit2 size={16}/></button>
                    <button onClick={() => setDeleteConfirmId(item.id)} className="p-2 bg-slate-50 rounded-xl text-red-300"><Trash2 size={16}/></button>
                  </div>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl mb-4 flex justify-between items-center">
                  <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Machine</p><p className="font-bold text-slate-700">{item.machine}</p></div>
                  <div className="text-right"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Stage</p><p className="font-black text-pink-500 uppercase text-xs tracking-tighter">{item.stage}</p></div>
                </div>
        ...
        <div className="hidden md:block bg-white/60 backdrop-blur-sm rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden opacity-95 mb-10">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50 text-[11px] font-bold uppercase tracking-widest text-slate-400 border-b"><tr><th className="p-6 text-left">Customer</th><th className="p-6 text-left">Machine</th><th className="p-6 text-center">Status</th><th className="p-6 text-right">Next Stage</th></tr></thead>
            <tbody className="divide-y divide-slate-50">
              {dashboardData.completed.map(item => (
                <tr key={item.id} className="hover:bg-slate-50/30 transition-colors group">
                  ...
                  <td className="p-6 flex items-center gap-2 justify-center text-[10px] font-bold uppercase text-emerald-600 tracking-widest"><CheckCircle size={14} className="text-emerald-500" /> {item.stage === "Newborn" ? "Growing Up" : "Card Sent"}</td>
                  <td className="p-6 text-[10px] font-bold uppercase text-slate-300 tracking-widest leading-tight text-right uppercase">{item.stage === "Newborn" ? "Toddler (Age 2)" : "Next Milestone"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="md:hidden space-y-3">
          {dashboardData.completed.map(item => (
            <div key={item.id} className="bg-white/70 p-5 rounded-3xl border border-slate-100 flex justify-between items-center">
              <div><p className="font-bold text-slate-700 text-sm">{item.customer}</p><p className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.1em]">{item.machine}</p></div>
              <div className="flex items-center gap-2 text-[9px] font-black uppercase text-emerald-500 bg-emerald-50 px-3 py-1.5 rounded-full"><CheckCircle size={10}/> {item.stage === "Newborn" ? "New" : "Sent"}</div>
            </div>
          ))}
        </div>
      </div>
