...
const getAppId = () => {
  ...
  const cleanId = String(rawAppId).split('/').filter(Boolean).pop() || 'machine_bday_crm';
  return cleanId.replace(/[^a-zA-Z0-9]/g, '_');
};
        ...
        <div className="hidden md:block bg-white/80 backdrop-blur-sm rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden mb-12">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {dashboardData.toSend.length === 0 ? ( <tr><td colSpan="4" className="p-24 text-center text-slate-300 font-bold italic text-lg uppercase opacity-50 underline decoration-slate-100">Queue Empty</td></tr> ) : (
          ...
          <div className="bg-white md:rounded-[2.5rem] shadow-2xl w-full max-w-xl min-h-screen md:min-h-0 overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 md:p-10 space-y-4 md:space-y-6 flex-1">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <input placeholder="CITY" className="bg-slate-50 border-2 border-slate-100 p-4 rounded-2xl font-black outline-none focus:border-indigo-500 text-sm uppercase" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} />
